...
public void OnPluginStart() {
  ...
  // convar setup
	...
	RegConsoleCmd("sm_server", Command_Servers, "Hop servers.");
...
public int Menu_Handler(Menu menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_Select) {
		...
		if (!g_bConnectedFromFavorites[param1]) {
			CPrintToChat(param1, "{blue}[{aqua}ServerHop{blue}] {white}Due to Valve game change, You must connect via favorites to be redirected by server.");
			CPrintToChat(param1, "{blue}[{aqua}ServerHop{blue}] %s:{white} %s", g_sServer[param1], g_sAddress[param1]);
			PrintHintText(param1, "Due to Valve game change, You must connect via favorites to be redirected by server.");
			return;
		}
